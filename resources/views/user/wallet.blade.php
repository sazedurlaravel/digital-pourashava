<x-user.app-layout>
    ...
    <x-slot name="header">
        <x-admin.content-header>
            ...
            <a onclick="wallet_form(' ই-ওয়ালেট', '{{ route('pourashava_frontend.user.wallet.request', Request::route('pourashava_slug')) }}')" data-toggle="modal" data-target="#walletForm" href="#" class="btn btn-primary"> <i class="fas fa-plus mr-1"></i> রিকুয়েস্ট এড মানি </a>
        </x-admin.content-header>
    </x-slot>
    ...
    <div class="card card-info">
        ...
        <div class="card-body">
            <h3 class="text-center"> টোটাল ব্যালেন্স : {{App\Helpers\Facades\WalletHelper::get_balance($guard='user', $wallets->first()->user)}} টাকা </h3>
                        ...
                        <td>
                            <div class="dropdown">
                                ...
                                <div  @if ($wallet->status == 0) class="dropdown-menu dropdown-menu-right"  @endif aria-labelledby="dropdownMenuLink">
                                    ...
                                    @if ($wallet->status == 0)
                                    <a class="dropdown-item" onclick="wallet_form('আপডেট ই-ওয়ালেট', '{{ route('pourashava_frontend.user.wallet.update' ,[Request::route('pourashava_slug'), $wallet->id]) }}','{{ $wallet->amount }}')" data-toggle="modal" data-target="#walletForm" href="#"> এডিট </a>
                                    @endif
                                </div>
                            </div>
                        </td>
